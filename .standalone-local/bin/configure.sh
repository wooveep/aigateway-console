...
DEFAULT_NACOS_NS=aigateway-system
  ...
  if [ "$USE_BUILTIN_NACOS" == "Y" ] || [ -n "$CONFIG_URL" ]; then
    if [ "$USE_BUILTIN_NACOS" == "Y" ]; then
      ...
      if [[ $NACOS_SERVER_URL == *"localhost"* ]] || [[ $NACOS_SERVER_URL == *"/127."* ]]; then
        echo "AIGateway will be running in a docker container. localhost or loopback addresses won't work. Please use a non-loopback host in the Nacos URL."
        exit -1
      fi
    fi
  else
...
configureStandaloneNacosServer() {
  while true; do
    ...
    if [[ $input == *"localhost"* ]] || [[ $input == *"/127."* ]]; then
      echo "AIGateway will be running in a docker container. localhost or loopback addresses won't work. Please use a non-loopback host in the URL."
      continue
    fi
...
configureConsole() {
  # echo "==== Configure AIGateway Console ===="
  :
}

configurePorts() {
  echo "==== Configure Ports to be used by AIGateway ===="
  ...
  readPortWithDefault "Please input the local HTTP port to access AIGateway Gateway [${DEFAULT_GATEWAY_HTTP_PORT}]: " ${DEFAULT_GATEWAY_HTTP_PORT}
  GATEWAY_HTTP_PORT=$input
  readPortWithDefault "Please input the local HTTPS port to access AIGateway Gateway [${DEFAULT_GATEWAY_HTTPS_PORT}]: " ${DEFAULT_GATEWAY_HTTPS_PORT}
  GATEWAY_HTTPS_PORT=$input
  readPortWithDefault "Please input the local metrics port to be listened by AIGateway Gateway [${DEFAULT_GATEWAY_METRICS_PORT}]: " ${DEFAULT_GATEWAY_METRICS_PORT}
  GATEWAY_METRICS_PORT=$input
  readPortWithDefault "Please input the local port to access AIGateway Console [${DEFAULT_CONSOLE_PORT}]: " ${DEFAULT_CONSOLE_PORT}
  CONSOLE_PORT=$input
}
...
outputUsage() {
  ...
  echo '
 -a, --auto-start           start AIGateway after configuration
 -c, --config-url=URL       URL of the config storage
                            Use Nacos with format: nacos://192.168.0.1:8848
                            Use local files with format: file:///opt/aigateway/conf
  ...
  \ \  \ \  \ \  \ \  \|\  \ \  \\  \\ \  \_|\ \|____|\  \\|____|\  \
   \ \__\ \__\ \__\ \_______\ \__\\ _\\ \_______\____\_\  \ ____\_\  \
    \|__|\|__|\|__|\|_______|\|__|\|__|\|_______|\_________\\_________\
                                                \|_________\|_________|
'
  echo "AIGateway is configured successfully."
  echo ""
...
runInitializer() {
  ...
  if [ "$COMPOSE_PROFILES" == "nacos" ]; then
    ...
    cd "$COMPOSE_ROOT" && runDockerCompose -p "$COMPOSE_PROJECT" up -d nacos
    retVal=$?
    if [ $retVal -ne 0 ]; then
  ...
  cd "$COMPOSE_ROOT" && runDockerCompose -p "$COMPOSE_PROJECT" run -T --rm initializer
  local retVal=$?
  if [ $retVal -ne 0 ]; then
    echo "AIGateway configuration failed with $retVal."
    exit -1
  fi

  if [ "$COMPOSE_PROFILES" == "nacos" ] && [ "${AUTO_START}" != "Y" ]; then
    echo "Stopping built-in Nacos service..."
    cd "$COMPOSE_ROOT" && runDockerCompose -p "$COMPOSE_PROJECT" down --remove-orphans
    local retVal=$?
    if [ $retVal -ne 0 ]; then
...
if [ -f "$CONFIGURED_MARK" ]; then
  if [ "$RERUN" == "Y" ]; then
    ...
    echo "AIGateway is already configured. Please add \"-r\" if you want to re-run the configuration workflow."
    exit -1
  fi
fi
configure
