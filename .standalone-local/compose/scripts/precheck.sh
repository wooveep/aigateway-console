...
checkApiServer() {
  ...
  openssl req -nodes -new -x509 -days 36500 -keyout ca.key -out ca.crt -subj "/CN=aigateway-root-ca/O=aigateway" >/dev/null 2>&1
  checkExitCode "  Generating CA certificate for API server fails with $?"

  echo "  Generating server certificate..."
  openssl req -out server.csr -new -newkey rsa:$RSA_KEY_LENGTH -nodes -keyout server.key -subj "/CN=aigateway-api-server/O=aigateway" >/dev/null 2>&1 &&
  ...
  openssl req -out client.csr -new -newkey rsa:$RSA_KEY_LENGTH -nodes -keyout client.key -subj "/CN=aigateway/O=system:masters" >/dev/null 2>&1 &&
...
contexts:
  - name: aigateway
    context:
      cluster: aigateway
      user: aigateway-admin
preferences: {}
current-context: aigateway
EOF
}
