...
SYSTEM_NAMESPACE="${NACOS_NS:-aigateway-system}"
INGRESS_CLASS_NAME="aigateway"
GATEWAY_SELECTOR_KEY="aigateway"
GATEWAY_RESOURCE_NAME="aigateway-gateway"
GATEWAY_SELECTOR_VALUE="${SYSTEM_NAMESPACE}-${GATEWAY_RESOURCE_NAME}"
CONSOLE_RESOURCE_NAME="aigateway-console"
CONSOLE_SERVICE_FQDN="${CONSOLE_RESOURCE_NAME}.dns"
CONSOLE_CONFIG_NAME="${CONSOLE_RESOURCE_NAME}"
GATEWAY_CUSTOM_RESPONSE_FILTER_NAME="aigateway-gateway-global-custom-response"
...
getConfig() {
  ...
  local tmpFile=$(mktemp /tmp/aigateway-precheck-config.XXXXXXXXX.cfg)
...
EOF
    ...
    if [ -n "$NACOS_USERNAME" ] && [ -n "$NACOS_PASSWORD" ]; then
      ...
      checkConfigExists "${SYSTEM_NAMESPACE}" "v1" "secrets" "$nacosAuthSecretName"
      if [ $? -ne 0 ]; then
...
type: Opaque
EOF
        publishConfig "${SYSTEM_NAMESPACE}" "v1" "secrets" "${nacosAuthSecretName}" "$nacosAuthSecretContent"
        read -r -d '' mcpbridgeContent <<EOF
${mcpbridgeContent}
...
EOF
  ...
  checkConfigExists "${SYSTEM_NAMESPACE}" "networking.higress.io/v1" "mcpbridges" "default"
  if [ $? -ne 0 ]; then
    echo "  The McpBridge resource \"default\" doesn't exist. Create it now..."
    publishConfig "${SYSTEM_NAMESPACE}" "networking.higress.io/v1" "mcpbridges" "default" "$mcpbridgeContent" true
  fi
}
...
L = Shanghai
O = AIGateway
CN = AIGateway Gateway

[v3_req]
keyUsage = digitalSignature, keyEncipherment
subjectAltName = URI:spiffe://cluster.local/ns/${SYSTEM_NAMESPACE}/sa/${GATEWAY_RESOURCE_NAME}
EOF
...
checkAigatewayConfig() {
  checkConfigExists "${SYSTEM_NAMESPACE}" "v1" "configmaps" "higress-config"
  if [ $? -ne 0 ]; then
...
data:
  ...
  mesh: |-
    ...
    rootNamespace: ${SYSTEM_NAMESPACE}
    trustDomain: cluster.local
  meshNetworks: 'networks: {}'
EOF
    publishConfig "${SYSTEM_NAMESPACE}" "v1" "configmaps" "higress-config" "$content"
  fi

  mkdir -p $VOLUMES_ROOT/pilot/config && cd "$_"
  getConfig "${SYSTEM_NAMESPACE}" "v1" "configmaps" "higress-config"
...
EOF
  ...
  checkConfigExists "${SYSTEM_NAMESPACE}" "networking.istio.io/v1alpha3" "envoyfilters" "${GATEWAY_CUSTOM_RESPONSE_FILTER_NAME}"
  if [ $? -ne 0 ]; then
    echo "  The EnvoyFilter resource \"${GATEWAY_CUSTOM_RESPONSE_FILTER_NAME}\" doesn't exist. Create it now..."
    read -r -d '' content <<EOF
apiVersion: networking.istio.io/v1alpha3
kind: EnvoyFilter
metadata:
  name: ${GATEWAY_CUSTOM_RESPONSE_FILTER_NAME}
  namespace: ${SYSTEM_NAMESPACE}
  labels:
    app: ${GATEWAY_RESOURCE_NAME}
    ${GATEWAY_SELECTOR_KEY}: ${GATEWAY_SELECTOR_VALUE}
...
EOF
    publishConfig "${SYSTEM_NAMESPACE}" "networking.istio.io/v1alpha3" "envoyfilters" "${GATEWAY_CUSTOM_RESPONSE_FILTER_NAME}" "$content"
  fi
}
...
checkConsole() {
  ...
  checkConfigExists "${SYSTEM_NAMESPACE}" "v1" "secrets" "${CONSOLE_CONFIG_NAME}"
  if [ $? -ne 0 ]; then
    echo "  The ConfigMap resource \"${CONSOLE_CONFIG_NAME}\" doesn't exist. Create it now..."
    read -r -d '' content <<EOF
apiVersion: v1
data:
...
metadata:
  creationTimestamp: "$(now)"
  name: ${CONSOLE_CONFIG_NAME}
  namespace: ${SYSTEM_NAMESPACE}
type: Opaque
EOF
    publishConfig "${SYSTEM_NAMESPACE}" "v1" "secrets" "${CONSOLE_CONFIG_NAME}" "$content"
  fi

  checkConfigExists "${SYSTEM_NAMESPACE}" "v1" "configmaps" "${CONSOLE_CONFIG_NAME}"
  if [ $? -ne 0 ]; then
    echo "  The Secret resource \"${CONSOLE_CONFIG_NAME}\" doesn't exist. Create it now..."
    read -r -d '' content <<EOF
apiVersion: v1
kind: ConfigMap
metadata:
...
data:
  mode: standalone
EOF
    publishConfig "${SYSTEM_NAMESPACE}" "v1" "configmaps" "${CONSOLE_CONFIG_NAME}" "$content"
  fi
}
...
checkGatewayApi() {
  ...
  checkConfigExists "${SYSTEM_NAMESPACE}" "v1" "services" "${GATEWAY_RESOURCE_NAME}"
  if [ $? -ne 0 ]; then
    echo "  The Service resource \"${GATEWAY_RESOURCE_NAME}\" doesn't exist. Create it now..."
    read -r -d '' content <<EOF
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  type: LoadBalancer
EOF
    publishConfig "${SYSTEM_NAMESPACE}" "v1" "services" "${GATEWAY_RESOURCE_NAME}" "$content"
  fi

  checkConfigExists "" "gateway.networking.k8s.io/v1beta1" "gatewayclasses" "${GATEWAY_RESOURCE_NAME}"
  if [ $? -ne 0 ]; then
    echo "  The GatewayClass resource \"${GATEWAY_RESOURCE_NAME}\" doesn't exist. Create it now..."
    read -r -d '' content <<EOF
...
EOF
    publishConfig "" "gateway.networking.k8s.io/v1beta1" "gatewayclasses" "${GATEWAY_RESOURCE_NAME}" "$content"
  fi

  checkConfigExists "${SYSTEM_NAMESPACE}" "gateway.networking.k8s.io/v1beta1" "gateways" "${GATEWAY_RESOURCE_NAME}"
  if [ $? -ne 0 ]; then
    echo "  The Gateway resource \"${GATEWAY_RESOURCE_NAME}\" doesn't exist. Create it now..."
    read -r -d '' content <<EOF
apiVersion: gateway.networking.k8s.io/v1beta1
kind: Gateway
metadata:
  ...
  namespace: ${SYSTEM_NAMESPACE}
spec:
  gatewayClassName: ${GATEWAY_RESOURCE_NAME}
  listeners:
  - name: http
    port: 80
    protocol: HTTP
  ...
  - name: https
    ...
    allowedRoutes:
      namespaces:
        from: All
EOF
    publishConfig "${SYSTEM_NAMESPACE}" "gateway.networking.k8s.io/v1beta1" "gateways" "${GATEWAY_RESOURCE_NAME}" "$content"
  fi
}
...
scrape_configs:
  ...
  - job_name: 'gateway'
    ...
    - targets: ['gateway:15020']
      labels:
        container: '${GATEWAY_RESOURCE_NAME}'
        namespace: '${SYSTEM_NAMESPACE}'
        ${GATEWAY_SELECTOR_KEY}: '${GATEWAY_SELECTOR_VALUE}'
        pod: '${GATEWAY_RESOURCE_NAME}'
EOF
