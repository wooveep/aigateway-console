...
initializeNacos() {
  ...
  maxWaitTime=180
  local tmpFile=$(mktemp /tmp/aigateway-init.XXXXXXXXX.cfg)
...
- Got ${v3StatusCode} from ${v3HealthCheckurl}"
  ...
  if [ "$NACOS_USE_RANDOM_DATA_ENC_KEY" == "N" ]; then
    ...
    if [ $statusCode -eq 200 ]; then
      echo "  ERROR: AIGateway configs are found in nacos namespace ${NACOS_NS}."
      echo
      echo "  Using a random data encyption key in a configured nacos namespace is incorrect, and will cause AIGateway unable to start."
      echo "  You can:"
      echo "  1. Remove all the configurations in nacos namespace ${NACOS_NS} and try again."
      echo "  2. Install AIGateway to another nacos namespace."
      echo "  3. Specify the same data encryption key generated/used in the previous installation."
      exit -1
    elif [ $statusCode -eq 404 ]; then
      echo "  No AIGateway config is found in nacos namespace ${NACOS_NS}."
    else
