...
public abstract class AbstractMcpServerSaveStrategy implements McpServerSaveStrategy {
    ...
    private Route saveRoute(McpServer mcpInstance) {
        ...
        if (Objects.isNull(existRoute)) {
            ...
            try {
                ...
                if (e.getCode() == HttpStatus.UNPROCESSABLE_ENTITY) {
                    throw new ValidationException(
                        StringUtils.defaultIfBlank(e.getResponseBody(), e.getMessage()));
                }
        ...
        try {
            ...
            if (e.getCode() == HttpStatus.UNPROCESSABLE_ENTITY) {
                throw new ValidationException(StringUtils.defaultIfBlank(e.getResponseBody(), e.getMessage()));
            }
            throw new BusinessException(
    ...
    protected Route buildRouteRequest(McpServer mcpInstance) {
        McpServerHelper.validateMcpServerName(mcpInstance.getName());
    ...
    protected void setDefaultConfigs(Route route, McpServer mcpInstance) {
        ...
        ConsumerAuthInfo consumerAuthInfo = mcpInstance.getConsumerAuthInfo();
        if (consumerAuthInfo != null) {
            String encodedLevels =
                RouteAuthConfig.encodeAllowedConsumerLevels(consumerAuthInfo.getAllowedConsumerLevels());
            if (StringUtils.isNotBlank(encodedLevels)) {
                annotationsMap.put(RouteAuthConfig.ANNOTATION_ALLOWED_CONSUMER_LEVELS, encodedLevels);
            }
        }
